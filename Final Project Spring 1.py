...
#default url without any routing as GET request (REFERENCE: from 5th lecture's py file)
@app.route('/', methods=['GET'])
def home():
    return "<h1> Welcome to Project Final Sprint 1! </h1>"
...
def add_airport_record():
    ...
    add_airport_query = "INSERT INTO airports (id,airportcode,airportname,country) VALUES ('%s', '%s', %s, %s)" % (id,airportcode,airportname,country)
...
def read_airports_records():
    ...
    sql = "SELECT * FROM airports WHERE id = %s" %(id)
...
def add_planes_record():
    ...
    add_planes_query = "INSERT INTO planes (id,make,model,year,capacity) VALUES ('%s', '%s', %s, %s)" % (id,make,model,year,capacity)
...
def read_planes_records():
    ...
    myCreds = creds.Creds()
    conn = create_connection(myCreds.conString, myCreds.userName, myCreds.password, myCreds.dbName)
    sql = "SELECT * FROM planes WHERE id = %s" %(id)
...
def add_flight_record():
    ...
    add_flight_query = "INSERT INTO flights (id,planeid,airportfromid,airporttoid,date) VALUES ('%s', '%s', %s, %s)" % (id,planeid,airportfromid,airporttoid,date)
